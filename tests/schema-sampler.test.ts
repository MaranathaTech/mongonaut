...
describe('SchemaSampler', () => {
  ...
  it('should extract nested field paths', async () => {
    mockToArray.mockResolvedValue([{ address: { city: 'NYC', zip: '10001' } }]);
  ...
  it('should extract array element fields', async () => {
    mockToArray.mockResolvedValue([{ tags: [{ label: 'important', priority: 1 }] }]);
  ...
  it('should detect correct types for each field', async () => {
    mockToArray.mockResolvedValue([{ name: 'Alice', age: 30, active: true, email: null }]);
  ...
  it('should detect array type', async () => {
    mockToArray.mockResolvedValue([{ items: [1, 2, 3] }]);
  ...
  it('should detect object type for nested docs', async () => {
    mockToArray.mockResolvedValue([{ meta: { version: 1 } }]);
