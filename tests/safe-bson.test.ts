...
vi.mock('bson', () => {
  const EJSON = {
    stringify: (value: unknown, options?: { relaxed?: boolean }) => {
    ...
    parse: (text: string, options?: { relaxed?: boolean }) => {
...
const { serializeDocuments, serializeDocument, deserializeDocument } = await import(
  '../src/main/lib/safe-bson'
)
